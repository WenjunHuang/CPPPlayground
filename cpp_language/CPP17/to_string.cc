...
std::string add_suffix_float(double val,const std::string& suffix) {
    if (val < 0.001) {
        std::string ret;
        ret.resize( 4 + 2,' ');
        if (!suffix.empty()) ret.resize(4 + 2 +suffix.length(),' ');
    ...
    std::snprintf(ret.data(),ret.max_size(),"%4.*f%s%s",
                  val < 99 ? 1 :0,
                  val,
                  prefix[i].data(),
...
int main() {
    std::cout << to_string(9282,10) << std::endl;
    std::cout << esc("39") << std::endl;

    std::cout << add_suffix_float(39192313,"") << std::endl;

    std::cout << color("abc",col_cyan) << std::endl;
    std::cout << progress_bar(1000,10,col_green,' ',' ',"hello",0) << std::endl;
    std::cout << progress_bar(1000,10,col_green,' ',' ',"",0) << std::endl;
