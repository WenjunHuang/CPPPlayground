...
struct LayoutGlyph {
  ...
  // moving this into a run length representation, because it's more efficient
  // for long strings, and we'll probably need something like that for paint
  // attributes (color, underline, fake b/i, etc), as having those per-glyph is
  // bloated.
  int font_ix;
  ...
  // libtxt extension: record the cluster (character index) that corresponds to
  // this glyph
  uint32_t cluster;
};
...
class Layout {
 public:
  Layout() : glyphs_(), advances_(), faces_(), advance_(0), bounds_() {
    bounds_.setEmpty();
  }
  ...
  // Get advances, copying into caller-provided buffer. The size of this buffer
  // must match the length of the string (count arg to doLayout).
  ...
  float getCharAdvance(size_t i) const { return advances_[i]; }
